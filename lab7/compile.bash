#!/bin/bash
# filepath: /home/gefgu/repos/riscv-microprocessor/lab7/compile.bash

# Set the working directory to lab7
cd "$(dirname "$0")"
...
echo -e "${GREEN}Starting compilation of RISC-V processor for lab7 (with RAM)...${NC}"
...
# Step 6: Compile the lab7 components (RAM)
echo -e "${GREEN}Compiling lab7 files (RAM)...${NC}"
ghdl -a RAM.vhd || { echo -e "${RED}Failed to compile RAM${NC}"; exit 1; }

# Step 7: Compile the testbench
echo -e "${GREEN}Compiling testbench...${NC}"
ghdl -a processador_lab7_tb.vhd || { echo -e "${RED}Failed to compile processador_lab7_tb${NC}"; exit 1; }

# Step 8: Build the executable
echo -e "${GREEN}Building the executable...${NC}"
ghdl -e processador_lab7_tb || { echo -e "${RED}Failed to build the executable${NC}"; exit 1; }

# Step 9: Run the simulation
echo -e "${GREEN}Running the simulation...${NC}"
ghdl -r processador_lab7_tb --wave=processador_lab7_tb.ghw

echo -e "${GREEN}Compilation and simulation completed successfully.${NC}"
echo -e "${GREEN}You can view the waveform using GTKWave: gtkwave processador_lab7_tb.ghw${NC}"